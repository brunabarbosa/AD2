...
train.clean$EVADIU <- as.factor(ifelse(train.clean$EVADIU == 0, "FALSE", "TRUE"))
...
lasso <- train(EVADIU ~ Cálculo.Diferencial.e.Integral.I
               + Introdução.à.Computação
               + Laboratório.de.Programação.I
               + Leitura.e.Produção.de.Textos
               + Programação.I
               + Álgebra.Vetorial.e.Geometria.Analítica, data = train.clean,
               method='lasso',
               metric = ifelse(is.factor(EVADIU), "Accuracy", "RMSE"),  
               trControl=fitControl)

ridge <- train(as.factor(EVADIU) ~ Cálculo.Diferencial.e.Integral.I
               + Introdução.à.Computação
               + Laboratório.de.Programação.I
               + Leitura.e.Produção.de.Textos
               + Programação.I
